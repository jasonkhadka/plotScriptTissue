...
#                       CLUSTER CODE 
#            Task :
#                  Growth of Cylinderical surface with Radial degree of freedom
#                   WITH CYLINDRICAL COORSINATE
#                  #Heavy Penalty for crossng bending threshold of 0.45
#            Initial condition :
#                        Uniform Dome
#            Bounds : 
#                   1. Dome is allowed to grow in X-Y-Z
#                   2. Cylindrical flanks are bound in X-Y with NO wiggle room but Z-coordinates are allowed to move by 1 side length up or down
#                   3. The floor vertices are fixed in X-Y-Z with NO wiggle room
#            Growth : 
#                   No growth 
...
parser.add_argument("-r","--redline", help="put red vertical line on the plots for each growth step",action="store_true")#if this argument is used then redline = True will be set, else default = False
...
for _ in range(numOfLayer):cell = latdev.makeLaterCylindricalLayer(cell,numOfLayer)
...
stepcounter = 0
###########################################################################################
#            Gathering Cylindrical Coordinates
############################################################################################
...
###########################################################################
#upperbounds, lowerbounds = getCylindricalBoundsForOptimization(cell,radius,perimeterVertexNum)
...
for growthcounter in range(growthSteps):
    ...
    ################################################################
    stepcounter = growthcounter + 1 # As stepcounter saves curernt figure but above growth counter is used to save figures from previous growth
    ...
    if np.isnan(initialenergy):
        ...
        ###################################################################################################
        print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        print "           Starting growth       STEP : : ", growthcounter
        print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        cell = inflatedGrowFaces(cell)
        ###############################
        ...
        upperbounds, lowerbounds = getCylindricalBoundsForOptimization(cell,hemisphereradius,perimeterVertexNum)
        opt.set_lower_bounds(lowerbounds)
        opt.set_upper_bounds(upperbounds)
        ...
        print "Fourth Term : ", cell.getFourthTerm()
        functionCallCounter = 0
        try:
        ...
        ###########################################################################################
        #   PUTING In The Vertices On The CELL  #
        ###########################################################################################
