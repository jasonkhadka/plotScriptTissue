...
#                   2. Cylindrical flanks are bound in X-Y with little wiggle room but Z-coordinates are allowed to move by 1 side lenght up or down
#                   3. The floor vertices are fixed in X-Y-Z with little wiggle room
#            Growth : 
#                   No growth 
...
totalvertex = cell.countVertices()
#####################################################
...
def termsofenergy(inputcoordinates):
    global numOfLayer, alpha, beta, pressure, cell
    #making of hexagonal lattice
    #maincounter += 1 
    #Reshape the tempcoordinates, to x-y-z arrays
    tempcoordinates = inputcoordinates.reshape((3,numberofvertices))
...
def getBoundsForOptimization(cell,radius, perimeterVertexNum):
    ...
    while vertex != None: 
        ...
        ## DOME VERTEX
        if currentvertid > perimeterVertexNum and currentvertid <= numberofvertices:
            ...
            lboundsz[vertcounter] = ((zcoord[vertcounter] - Length)>0)*(zcoord[vertcounter] - Length)#z-coordinates are allowed to move by 1 side lenght up or down or until z >= 0
        vertcounter += 1
        vertex = vertices.next()
    ###base cells###
    vertices = qd.CellVertexIterator(cell)
...
################# Upper bounds #################
###base cells###
boundsx[(-1*perimeterVertexNum):] = basefreedom
boundsy[(-1*perimeterVertexNum):] = basefreedom
boundsz[(-1*perimeterVertexNum):] = zcoord[(-1*perimeterVertexNum):]+zbasefreedom
###Upper cells ###
boundsx[:(-1*perimeterVertexNum)] = basefreedom
boundsy[:(-1*perimeterVertexNum)] = basefreedom
boundsz[:(-1*perimeterVertexNum)] = basefreedom
