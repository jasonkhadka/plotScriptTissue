...
#           ** This code bounds the cells with center near x axis (-0.5 <= x-coordinate <= 0.5), so that they cant move
#           ** Essentially simulating infinite stiffness on these cells, and see if we can create two domes
#           ** 
...
stiffnessfactor = 1000#the stifness factor to use for x-band cells
...
if args.cylinder: 
	...
	cell = setTFMMatchedCartesianCoordinate(cell,numOfLayer)
	cellradius = radius
############################################################################################
...
####################################################################################
# Change Stiffness of the Band of cells arround x = 0 and Plot it
####################################################################################
print "Stiffness is increased for X - band cells by factor of 10"
cell = changeStiffnessXBand(cell,factor=stiffnessfactor)
plotFaceStiffness(cell,numOfLayer, step =0)#Plotting & saving the stiffness plot
####################################################################################
...
def cartesianEnergyObjective(inputcoordinates,grad):
		...
		return energyvalue
############################################################################
###                 Function to bound X-band cells                       ###
############################################################################
def checkXBand(vertex):
		edges = qd.VertexEdgeIterator(vertex)
		edge = edges.next()
		while edge != None:
				face = edge.Left()
				if abs(face.getXCentralised())<=0.5:
						#If true : this vertex belongs to x-band faces
						return True
				edge = edges.next()
		#returning false : if this vertex has no face that belongs to x-band
		return False
############################################################################
###                 Function to bound X-band cells                       ###
############################################################################
...
for growthcounter in range(growthSteps):
		...
		if np.isnan(initialenergy):
				...
				## Bounding cells on the X-band
				#upperbounds, lowerbounds = getXBandBoundsForOptimization(cell,cellradius,perimeterVertexNum)
