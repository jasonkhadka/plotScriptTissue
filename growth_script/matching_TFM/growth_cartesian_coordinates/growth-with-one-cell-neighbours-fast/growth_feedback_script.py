...
newpath = r'a=%(alpha).4f_g=%(gamma).4f_n=%(eta).4f_z=%(zeta).4f_s=%(sigma).4f_layer=%(layer).1d'%{"strain":initialStrain,"alpha":alpha,"beta":beta,"sigma":sigma, "gamma":gamma,"zeta":zeta,"eta":eta,"time":totaltime,"layer":numOfLayer}
...
for growthcounter in range(growthSteps):
		...
		if np.isnan(energyarray[-1]):
				...
				plotParametersAfterRelaxation(cell,numOfLayer,energyarray, firsttermarray, secondtermarray, 
																			...
																			fourthtermarray,relaxation_time_length, functionCallCounterArray,divisionCounter,sumWallLength)
				#######################################################################################################################
				matrixdictionary = plotMatrixDictionary(cell,faceidarray, matrixdictionary)
				###################################################################################################
