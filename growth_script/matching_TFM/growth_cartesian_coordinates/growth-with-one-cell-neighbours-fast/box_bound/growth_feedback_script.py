...
#           ** BOUNDS :: Box-Bounds on the external vertex
...
def targetFaceGrow(cell,faceid):
		...
		while (face != None):
				if face.getID() == faceid:
						print "###################################################################################################"
						print "Growing face %d & the neighbourhoodwith kappa : %.3f and growthvar : %.3f"%(faceid,face.getKappa(), face.getGrowthVar())
						print "###################################################################################################"
...
for growthcounter in range(growthSteps):
		...
		if np.isnan(initialenergy):
				...
				upperbounds, lowerbounds = getBoxBoundsForOptimization(cell,cellradius,perimeterVertexNum)
