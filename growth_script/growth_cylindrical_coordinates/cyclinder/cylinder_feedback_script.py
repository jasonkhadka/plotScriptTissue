...
varypressure = args.varyfeedback // 5 
varyfeedback = args.varyfeedback % 5 
gamma = (10.**(-3.))*10.**(varypressure)
if varyfeedback == 1:
    eta = 0.# for varyfeedback = 1 -> 0 Feedback
else:
    eta = (10.**(-5.))*10.**(varyfeedback) #varrying feedback here for 4 orders of magnitude
##################################################################################################
...
newpath = r'a=%(alpha).4f_g=%(gamma).4f_n=%(eta).4f_n=%(eta).4f_is=%(strain).2f_layer=%(layer).1d'%{"strain":initialStrain,"alpha":alpha,"beta":beta,"gamma":gamma,"zeta":zeta,"eta":eta,"time":totaltime,"layer":numOfLayer}
