...
#######################################################################################################################################
#               Importing the varinace coordinates
#######################################################################################################################################
varianceCoordinates = np.load("/home/jkhadka/variance_minimum_coordinates/layer%(layer).npy"%{"layer":layer})
varianceCoordinates = varianceCoordinates.reshape((3,len(varianceCoordinates)/3))
#######################################################################################################################################
#               Making Directory
#######################################################################################################################################
...
if args.shape:#if used -s / --shape option : identity matrix is set as intiial TargetMatrix
    ...
    os.chdir("relaxation_m0=identity")
else:
    if not os.path.exists("m0=m"):
         os.makedirs("m0=m")
    os.chdir("relaxation_m0=m")
#with_dome directory
...
if args.unlimited:
    ...
    newpath = r'a=%(alpha).4f_b=%(beta).4f_p=%(pressure)f_t=%(time)s_layer=%(layer).1d'%{"alpha":alpha,"beta":beta,"pressure":pressure,"time":"unlimited","layer":numOfLayer}
else:
    #now making path for simulation result
    newpath = r'M0=identity_a=%(alpha).4f_b=%(beta).4f_p=%(pressure).4f_t=%(time).4f_layer=%(layer).1d'%{"alpha":alpha,"beta":beta,"pressure":pressure,"time":totaltime,"layer":numOfLayer}
...
while face1 != None:
    ...
    face1 = faces.next()
##########################################################################################################
#setting the coordiantes from read file
vertices = qd.CellVertexIterator(cell)
vertex = vertices.next()
counter = 0#counter to change vertex positions
while vertex != None:
    vertex.setXcoordinate(varianceCoordinates[0,counter])#setting x coordinate
    vertex.setYcoordinate(varianceCoordinates[1,counter])#setting y coordinate
    vertex.setZcoordinate(varianceCoordinates[2,counter])#setting z coordinate
    counter += 1
    vertex = vertices.next()
########################################################################################################################
#####################################################
...
############################################################################
def endofprogram(txopt, totalenergy, firsttermarray, secondtermarray, thirdtermarray):
    ...
    np.savetxt("energyterms.txt", np.c_[totalenergy, firsttermarray, secondtermarray, thirdtermarray])
    return
############################################################################
...
latdev.plotSurface(cell, numOfLayer,name = r'surface_time={0:06d}.png'.format(0))
...
if np.isnan(initialenergy):
    ...
    for intervalpassed in xrange(totaltime/interval):
        ...
        np.savetxt(r'coordinates_after_step={0:06d}'.format(int(currenttime)),xopt)
        ...
        while vertex != None:
            ...
            vertex = vertices.next()
        #saving the plot
        latdev.plot3DCell(cell,name=r'time=%(time)d.png'%{"time":currenttime})
        #plotting the surface only of cell
        latdev.plotSurface(cell, numOfLayer,name = r'surface_time={0:06d}.png'.format(int(currenttime)))
...
endofprogram(xopt, energyarray, firsttermarray, secondtermarray, thirdtermarray)#saving the last relaxed coordinates
#printmatrixenergy(cell)
