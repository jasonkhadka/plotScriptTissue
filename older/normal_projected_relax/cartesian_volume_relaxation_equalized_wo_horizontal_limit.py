...
if not args.flat:#the dome is constructed in case the flat option is flase
    loadedverts = np.load("/home/jkhadka/equalized_coordinates/layer5_equalized.npy")
...
boundsz[(-1*perimeterVertexNum):] = zcoord[(-1*perimeterVertexNum):]+basefreedom
...
############################################################################
def endofprogram(txopt, totalenergy, firsttermarray, secondtermarray, thirdtermarray, volumearray, error = False):
    ...
    if not error:
        ...
        #energyterms = np.vstack((firsttermarray, secondtermarray, thirdtermarray))
        np.savetxt("ERROR_energyterms.txt", np.c_[totalenergy, firsttermarray, secondtermarray, thirdtermarray, volumearray])
    return
############################################################################
...
if np.isnan(initialenergy):
    ...
    for intervalpassed in xrange(totaltime/interval):
        ...
        plt.savefig('mean_deformation_plot.png'.format(int(stepcounter)), transparent = True)
        plt.clf()
        #closing all figures
        plt.close('all')
        #printmatrixenergy(cell)
