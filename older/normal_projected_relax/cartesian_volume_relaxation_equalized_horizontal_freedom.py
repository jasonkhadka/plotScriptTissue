####################################################################################
#                       CLUSTER CODE 
#                       starting from coordiantes of Equalized dome         #
...
if not args.flat:#the dome is constructed in case the flat option is flase
    loadedverts = np.load("/home/jkhadka/equalized_coordinates/layer{0:d}_equalized.npy".format(int(numOfLayer)))
...
def endofprogram(txopt, totalenergy, firsttermarray, secondtermarray, thirdtermarray, volumearray,fourthtermarray, error = False):
    ...
    if not error:
        ...
        np.savetxt("energyterms.txt", np.c_[totalenergy, firsttermarray, secondtermarray, thirdtermarray, volumearray, fourthtermarray])
    else:
        np.save("ERROR_final_coordiantes",tempcoordatearray)
        #energyterms = np.vstack((firsttermarray, secondtermarray, thirdtermarray))
        np.savetxt("ERROR_energyterms.txt", np.c_[totalenergy, firsttermarray, secondtermarray, thirdtermarray, volumearray,fourthtermarray])
    return
############################################################################
...
latdev.plotSurface(cell, 1,name = r'surface_time={0:d}.png'.format(1))
...
plt.savefig('deformation_plot_1.png', transparent = True)
plt.clf()
###########################################################################################
###########################################################################################
#start of simulation ::
np.savetxt(r'coordinates_after_step={0:d}'.format(int(stepcounter)),xopt)#saving initial coordinates
...
if np.isnan(initialenergy):
    ...
    while True:
        ...
        latdev.plotSurface(cell, 1,name = r'surface_time={0:d}.png'.format(int(stepcounter)))
