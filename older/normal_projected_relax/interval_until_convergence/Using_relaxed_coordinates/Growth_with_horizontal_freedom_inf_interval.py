...
####
##loadding the equalized coordinates
loadfilename = "/home/jkhadka/equalized_coordinates/layer{0:d}_equalized_rescaled.npy".format(int(numOfLayer))
if not args.flat:#the dome is constructed in case the flat option is flase
    loadedverts = np.load(loadfilename)
    #loadedverts = np.load("/home/jkhadka/equalized_coordinates/layer{0:d}_equalized_rescaled.npy".format(int(numOfLayer)))
    #putting the coordiantes in QUADEDGE
    vertices = qd.CellVertexIterator(cell)
    vert = vertices.next()
    vertcounter = 0
    while vert != None:
        vert.setXcoordinate(loadedverts[vertcounter][0])
        vert.setYcoordinate(loadedverts[vertcounter][1])
        vert.setZcoordinate(loadedverts[vertcounter][2])
        #print vert.getID()
        #print layercounter
        vert = vertices.next()
        vertcounter += 1
cell = settingFirstParameters(cell)#setting all the parameters of the cell
##loading the RELAXED COORDINATES
loadfilename = "/home/jkhadka/relaxed_coordinates/layer{0:d}/".format(int(numOfLayer))
loadfilename += newpath+"/relaxed_coordinates.npy"#directory name with all the calculation parameters
if not args.flat:#the dome is constructed in case the flat option is flase
    loadedverts = np.load(loadfilename)
    tempcoordinates = loadedverts.reshape((3,int(len(loadedverts)/3)))
    #print loadedverts
    #putting the coordiantes in QUADEDGE
    vertices = qd.CellVertexIterator(cell)
    vertex = vertices.next()
    vertcounter = 0#counter to change vertex positions
    while vertex != None:
        vertex.setXcoordinate(tempcoordinates[0,vertcounter])#setting x coordinate
        vertex.setYcoordinate(tempcoordinates[1,vertcounter])#setting y coordinate
        vertex.setZcoordinate(tempcoordinates[2,vertcounter])#setting z coordinate
        vertcounter += 1
        vertex = vertices.next()
########################################################################################################################
#####################################################
cell = settingParameters(cell)#setting all the parameters of the cell
######################################################
...
##stop criteria of tolerance for change in coordinates
opt.set_xtol_abs(tolerance)##so if all the coordinates move less than tolerance : criteria is fullfiled
...
for growthcounter in range(growthSteps):
    ...
    if np.isnan(initialenergy):
        ...
        print "###########################################################################################"
        print "Parameters For the Cells"
        print "###########################################################################################"
        print "alpha : ", cell.getAlpha()
        print "beta : ", cell.getBeta()
        print "pressure : ", cell.getPressure()
        ...
        """
        if (abs((energyarray[-1] - energyarray[-2])/energyarray[-2]) <= tolerance) or stepcounter >= totaltime: 
            print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        ...
        #performing growth
        cell = growFaces(cell)
        cell = settingParameters(cell)#setting new parameters after growth
