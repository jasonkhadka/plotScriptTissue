...
initialstep = 10.**(-3)
growthSteps = 20 #performing 1000 growth steps
...
# Projection condition is to have the external vertex projected on to an external circle
cell, newedge = latdev.makeCenteredHexagonLattice(numOfLayer, True, True) 
...
while face1 != None:
    ...
    face1 = faces.next()
##########################################################################################################
#          Making the flat surface into Spherical Tissue
##########################################################################################################
print "Making Crappy Spherical Tissue"
vertices = qd.CellVertexIterator(cell)
vertex = vertices.next()
while vertex != None:
    xtemp = vertex.getXcoordinate()
    ytemp = vertex.getYcoordinate()
    ztemp = np.sqrt(np.abs(radius**2 - xtemp**2 - ytemp**2))
    vertex.setZcoordinate(ztemp)
    vertex = vertices.next()
########################################################################################################################
#####################################################
...
opt.set_xtol_abs(tolerance/100.)
...
for growthcounter in range(growthSteps):
    ...
    if np.isnan(initialenergy):
        ...
        while True:
            ...
            print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
            ########################################################################################
            #caculating the error tolerance of energy
            if abs(optimizedenergy) < 1.:#incase optimized energy is less than 1, we look at absolute energy change
                energyerror = abs((energyarray[-1] - energyarray[-2]))
                energytolerance = tolerance/100.
                tolerancecondition = "Absolute"
            else:#incase optimized energy is more than 1, we look at the relative energy change
                energyerror = abs((energyarray[-1] - energyarray[-2])/min(energyarray[-1],energyarray[-2]))
                energytolerance = tolerance
                tolerancecondition = "Relative"

            print "time elapsed : ", time.time()-programstart
            if returncode in [1,2,3,4,5] and energyerror < energytolerance: #if return code is one of these then it makes Succesful convergence
                print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
                print "tolerance reached", "YES ! ", "SUCCESS !"
                optimizationTermination(returncode)
                print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
                print "Energy error : ", energyerror , "\n",tolerancecondition, " Energy Tolerance : ", energytolerance
        ...
        #performing growth
        print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        print "           Now starting growth       STEP : : ", growthcounter+1
        print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        cell = growFaces(cell)
        ###############################
