...
initialstep = 10.**(-3)
growthSteps = 1 #performing 1000 growth steps
...
####################
#here, is jobname is given then new folder is named by the jobname
if jobname != None: #it means jobname is given
    newpath = jobname+"g="+str(gamma)
#if jobname is not given then go by the similar previous algorithm name
...
zbasefreedom = (1./20.)*(2.*np.pi*radius)/perimeterVertexNum#vertex is allowed to move 5% of average perimeter side length
...
################# Upper bounds #################
###base cells###
boundsx[(-1*perimeterVertexNum):] = xcoord[(-1*perimeterVertexNum):]+zbasefreedom
boundsy[(-1*perimeterVertexNum):] = ycoord[(-1*perimeterVertexNum):]+zbasefreedom
...
boundsx[:(-1*perimeterVertexNum)] = -basefreedom
boundsy[:(-1*perimeterVertexNum)] = -basefreedom
boundsz[:(-1*perimeterVertexNum)] = -zbasefreedom
...
for growthcounter in range(growthSteps):
    ...
    if np.isnan(initialenergy):
        ...
        while True:
            ...
            plt.clf()
            #####stopping program after one iteration #######
            break
print "done"
