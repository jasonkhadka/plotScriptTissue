...
cell.setGamma(gamma)
cell.setKappa(kappa)
#setting new division factor for division threshold of faces
cell.setDivisionFactor(0.9)
#########    Printing Parameters    ########################
...
print "divisionFactor :", cell.getDivisionFactor()
print "gamma :",  gamma
###################################################################
...
def divideCells(cell):
    ...
    return cell
############################################################################
###        Function to Get coordiantes list of the cell                  ###
############################################################################
...
def termsofenergy(inputcoordinates):
    ...
    deformations = []
    ####iterating the vertices to feed the new coordinates in
    print "terms of energy inside started", "vertex count : ", numberofvertices
    vertices = qd.CellVertexIterator(cell)
    vertex = vertices.next()
    counter = 0#counter to change vertex positions
    while vertex != None:
        ...
        vertex.setZcoordinate(tempcoordinates[2,counter])#setting z coordinate
        counter += 1
        print "vertex ID ", vertex.getID()
        vertex = vertices.next()
    #####################################################
    #cell = settingParameters(cell)
    print "setting parameters of cell"
    cell.setParameters()
    #####################################################
    #calculating the deformation of Area 
    print " deformation area"
    faces = qd.CellFaceIterator(cell)
    face1 = faces.next()
    while face1 != None:
        if face1.getID() == 1: 
        ...
        deformations.append(currentarea)
        face1 = faces.next()
    print "done terms of energy"
    ######################################################
    #returning the total energy
...
def getBoundsForOptimization(cell,radius, perimeterVertexNum):
    ...
    while vertex != None: 
        ...
        ## DOME VERTEX
        if not checkExternalVertex(vertex):#so if vertex is external vertex, "else" is applied
            ...
            uboundsz[vertcounter] = zcoord[vertcounter]+epsbasefreedom#z-coordinates are allowed to move by 1 side lenght up or down
...
print "##############################################################################################################################################"
print "now Dividing Cells!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
print "##############################################################################################################################################"
cell = divideCells(cell)
print "Current Cell Division Number : ", cell.getDivisionCounter()
print "Current Vertex Count : ", cell.countVertices()
...
###################################################################################
##########         Coordinates and Bounds on relaxation                  ##########
###################################################################################
...
for growthcounter in range(growthSteps):
    ...
    if np.isnan(initialenergy):
        ...
        plt.plot(relaxation_time_length)
        plt.savefig('relaxation_time_step.png', transparent=True)
        plt.clf()
        ################################################################################################
        ###############################################################################################
        ##########         Resetting the Optimizer as vertices number changes after division  #########
        ###############################################################################################
        numberofvertices = cell.countVertices()
        opt = nlopt.opt(nlopt.LN_SBPLX,numberofvertices*3)
        xopt = getCoordinates(cell)
        ###########################################################################
        ##########         Conditions on the relaxation                  ##########
        ###########################################################################
        upperbounds, lowerbounds = getBoundsForOptimization(cell,radius,perimeterVertexNum)
